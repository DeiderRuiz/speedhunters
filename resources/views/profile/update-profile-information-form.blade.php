<x-form-section submit="updateProfileInformation">
    <x-slot name="title">
        <span class="dark:text-gray-300">{{ __('Información Del Perfil') }}</span>
    </x-slot>

    <x-slot name="description">
        <span class="dark:text-gray-300">{{ __('Actualiza la información de perfil y la dirección de correo electrónico de Tu cuenta.') }}</span>
    </x-slot>
            ...
            @if (Laravel\Fortify\Features::enabled(Laravel\Fortify\Features::emailVerification()) && ! $this->user->hasVerifiedEmail())
                <p class="text-sm mt-2">
                    ...
                    <button type="button" class="underline text-sm text-gray-600 hover:text-gray-900 rounded-md focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500" wire:click.prevent="sendEmailVerification">
                        {{ __('Click aquí para reenviar tu dirección de email.') }}
                    </button>
                </p>

                @if ($this->verificationLinkSent)
                    <p class="mt-2 font-medium text-sm text-green-600">
                        {{ __('Un nuevo link de verificación se envií a tu dirección de email.') }}
                    </p>
                @endif
            @endif
        </div>
